function [V,I,B] = readNODE_top(filename)
  % READNODE_TOP Read vertex positions from .node file, .node files are used by
  % Stellar and Triangle
  % and also identifies the boundary vertices.
  % Modified from the TRIANGLE function from gptoolbox by Alec Jacobson.
  % 
  % [V,I,B] = readNODE(filename)
  %
  % Input:
  ...
  %  B  list of bits indicating whether vertices are on the
  %   boundary
  %
